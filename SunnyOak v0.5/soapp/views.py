...
GPIO.add_event_detect(pinBtn, GPIO.FALLING, callback=bouton)
...
def index():
	...
	if lampeallume == True:
		...
		switchetat=''	
	sendmain = render_template('main.html', hue=hue, switchetat=switchetat)

	resp = Response(sendmain)
	resp.headers["Content-Type"] = 'text/html'
	resp.headers["Cache-Control"] = 'no-store'
	resp.headers["Refresh"] = '300; url=.'
	return resp
...
# Commande pour une des séquences
			...
			sendjson = json.dumps({"success": True, "command": request.form["command"], "img": True})
# Commande pour une des séquences
			...
			processes_sequence.append(process)
			sendjson = json.dumps({"success": True, "command": request.form["command"], "img": True})
# si la boucle if fonctionne pas
		else :
